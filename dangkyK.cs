...
namespace DOAN
{
    public partial class dangkyK : Form
    {
        public dangkyK()
        {
            InitializeComponent();
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (mod.TaikhoanKs("Select * from TaikhoanK where Gmail = '" + email + "'").Count != 0)
            {
            ...
            else
            {
                try
                {
                    string query = "Insert into TaikhoanK values('" + newtk + "','" + newmk + "','" + email + "')";
                    mod.Command(query);
